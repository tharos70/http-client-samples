...
public class Post {
    ...
    private static final Logger LOG = Logger.getLogger("post");
    ...
    public void simplePostCall(String url, String jsonPayload) throws IOException {
        LOG.info("Post:simplePostCall - IN");
        HttpUrl.Builder urlBuilder = HttpUrl.parse(url).newBuilder();
        RequestBody payload = RequestBody.create(jsonPayload, okhttp3.MediaType.parse("application/json"));
        Request request = new Request.Builder().url(urlBuilder.build()).post(payload).build();
        ...
        LOG.info("Post:simplePostCall - OUT");
    }
}